...
class ConfusionMatrix:
    ...
    def plot(self, save_dir='', names=()):
        try:
            ...
            #############################################################################
            ####################   confusion matirx csv 파일 추가    #####################
            #############################################################################
            df = pd.DataFrame(array)
            df.to_csv(save_dir / 'confusion_matrix.csv', index = True)

            fig = plt.figure(figsize=(12, 9), tight_layout=True)
            sn.set(font_scale=1.0 if self.nc < 50 else 0.8)  # for label size
            labels = (0 < len(names) < 99) and len(names) == self.nc  # apply names to ticklabels
            # annot=self.nc < 50 : 카테고리 개수가 50 미만이면 히트맵에 수치가 표현안됨
            sn.heatmap(array, annot=self.nc < 50, annot_kws={"size": 8}, cmap='Blues', fmt='.2f', square=True,
